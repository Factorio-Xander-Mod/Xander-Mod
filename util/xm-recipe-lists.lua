...
do
	local names = {"chalcocite", "molybdenite", "saltpeter", "borax", "lithium-concentrate", "chalcopyrite", "pentlandite", "hematite", "pyrolusite", "clay-a", "clay-b", "magnetite", "chromite", "sphalerite",
		"galena", "hydroxylapatite", "fluorite", "ilmenite", "zircon", "cassiterite", "scheelite", "monazite", "pitchblende"}
	local icons = {"__xander-mod__/graphics/recipe/raw/chalcocite-washing.png", "__xander-mod__/graphics/recipe/raw/chalcocite-washing.png", "__xander-mod__/graphics/recipe/salt/water-saltpeter.png",
	...
	local categories = {"sluice", "sluice", "sluice", "refining", "chemistry", "sluice", "sluice", "sluice", "sluice", "basic-sluice", "sluice", "sluice", "sluice", "sluice", "sluice", "sluice", "sluice",
		"sluice", "sluice", "sluice", "sluice", "sluice", "sluice"}
	local times = {20, 40, 4, 40, 10, 20, 20, 20, 20, 1, 20, 20, 40, 40, 20, 40, 40, 40, 40, 20, 40, 40, 20}
	local enableds = {false, false, false, false, false, false, false, false, false, true, false, false, false, false, false, false, false, false, false, false, false, false, false}
	local ingredients = {{{"milled-copper-ore", 10}, {type = "fluid", name = "water", amount = 50}}, {{"milled-copper-ore", 10}, {type = "fluid", name = "water", amount = 50}},
	...
	local results = {{{"chalcocite", 10}, {"gravel", 2}, {type = "fluid", name = "gray-water", amount = 50}}, {{"molybdenite", 5}, {"gravel", 2}, {type = "fluid", name = "gray-water", amount = 50}},
	...
	--Load all of the single-parameter lists into one big list of recipes, where each entry has the parameters for one complete recipe
	for i, name in ipairs(names) do
		table.insert(xm_concentrate_recipes, {name, icons[i], categories[i], {primary = {r = 0.48, g = 0.34, b = 0.24}, secondary = {r = 0.5, g = 0.5, b = 0.5}, tertiary = {r = 0.75, g = 0.75, b = 0.75}},
			times[i], enableds[i], ingredients[i], results[i], "concentrate"})
	end
end
...
xm_build_recipes = {}
do
	local names = {"stone-brick", "brick-clay-a", "brick-clay-b", "brick-clay-graphite-a", "brick-clay-graphite-b", "brick-spinel", "brick-zirconia", "cement-a", "cement-b", "concrete", "hazard-concrete",
		"reverse-hazard", "basalt-a", "basalt-b", "fused-quartz", "glass-a", "glass-b", "landfill", "landfill-b", "modified-a", "modified-b"}
	local categories = {"basic-machine", "kiln", "forge", "smelting", "forge", "forge", "forge", "refining", "refining", "sluice", "crafting", "crafting", "forge", "forge", "forge", "smelting", "forge", "sluice",
		"sluice", "sluice", "sluice"}
	local tints = {nil, nil, nil, nil, nil, nil, nil, nil, nil, {primary = {r = 0.48, g = 0.34, b = 0.24}, secondary = {r = 0.5, g = 0.5, b = 0.5}, tertiary = {r = 0.75, g = 0.75, b = 0.75}},
	...
	local times = {2, 4, 20, 4, 4, 5, 100, 20, 20, 10, 0.1, 0.1, 50, 50, 5, 40, 200, 1, 1, 4, 4}
	local enableds = {true, true, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false}
	local ingredients = {{{"stone", 2}}, {{"clay", 1}}, {{"clay", 8}, {"sand", 2}, {"calcium-oxide", 1}}, {{"clay", 1}, {"graphite-powder", 2}}, {{"clay", 1}, {"graphite-powder", 2}, {"sand", 1}},
	...
	local results = {{{"stone-brick", 1}}, {{"brick-clay", 1}}, {{"brick-clay", 10}}, {{"brick-clay-graphite", 1}}, {{"brick-clay-graphite", 2}}, {{"brick-spinel", 1}}, {{"brick-zirconia", 10}},
	...
	--Load all of the single-parameter lists into one big list of recipes, where each entry has the parameters for one complete recipe
	for i, name in ipairs(names) do
		table.insert(xm_build_recipes, {name, nil, categories[i], tints[i], times[i], enableds[i], ingredients[i], results[i], nil})
	end
end
